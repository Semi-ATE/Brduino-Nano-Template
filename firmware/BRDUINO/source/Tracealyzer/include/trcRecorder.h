...
#ifdef TRC_CFG_DFM_ENABLED
	/* DO NOT CHANGE */
	#define TRACE_SNAPSHOT_VERSION 6 /* Version 6: Added support for DFM. */
	#define TRACE_DFM_VERSION 1
#else
	/* DO NOT CHANGE */
...
/*******************************************************************************
 * The DFM header, needed by the Device Firmware Monitor cloud service.
 ******************************************************************************/
typedef struct
{
	 uint32_t symptom_id;
...
typedef struct
{
	uint16_t dfm_payload_size;
	uint16_t dfm_firmware_version_size;

	uint8_t enabled;
	uint8_t version;		 /* The version of the DFM subsystem. Initially 1. */
	uint8_t alert_type;		 /* The alert type, 0 if no alert is set */
	uint8_t symptom_count;   /* The number of registered symptoms. Must be < DFM_MAX_SYMPTOMS */
} dfm_header_t;

//typedef struct
//{
//	#if (TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN > 0)
//	char firmware_version[TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN];
//	#endif
//
//	#if (TRC_CFG_DFM_MAX_SYMPTOMS > 0)
//	dfm_symptom_t symptoms[TRC_CFG_DFM_MAX_SYMPTOMS];
//	#endif
//} dfm_payload_t;
typedef struct
{
...
#define dfm_init()
#define dfm_set_firmware_version(v)
#define dfm_alert(t)
#define dfm_alert_reset()
#define dfm_add_symptom(s, v)
...
typedef struct
{
	...
	/* DFM support, should be placed here before any symbols are read */
#if (TRACE_SNAPSHOT_VERSION >= 6)
	dfm_payload_t dfm_payload;
#endif
