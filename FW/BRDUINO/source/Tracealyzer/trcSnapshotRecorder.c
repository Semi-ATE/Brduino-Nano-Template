/*******************************************************************************
 * Trace Recorder Library for Tracealyzer v4.3.3
...
#if defined(TRC_CFG_DFM_ENABLED) && (TRC_CFG_DFM_ENABLED == 1)
	dfm_init();
#endif
...
{	
	...
	RecorderDataPtr->minor_version = TRACE_SNAPSHOT_VERSION;
...
void prvTraceError(const char* msg)
{
	...
	if (RecorderDataPtr != NULL)
	{
		// TODO: Insert dummy event, just before stopping the recorder, in order to set a final timestamp.
...
/* Shall be called from vTraceEnable (if DFM is enabled), i.e. inside critical section. */

#if (TRACE_SNAPSHOT_VERSION >= 6)

#ifndef TRC_CFG_DFM_ENABLED
#error "DFM not enabled but version say it should be"
#endif

#if (TRC_CFG_DFM_ENABLED >= 1)
	#ifndef TRC_CFG_DFM_MAX_SYMPTOMS
	#error "Max symptoms not defined"
	#endif

	#ifndef TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN
	#error "Max firmware version length not defined"
	#endif
#endif

void dfm_init()
{
	dfm_header_t* dfm = (dfm_header_t*)&(RecorderDataPtr->dfm_header);

	dfm->dfm_payload_size = sizeof(dfm_payload_t);

	#if TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN > 0
	dfm->dfm_firmware_version_size = TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN;
	#else
	dfm->dfm_firmware_version_size = 0;
	#endif

	dfm->enabled = TRC_CFG_DFM_ENABLED;
	dfm->version = TRACE_DFM_VERSION;
	dfm->alert_type = 0;
	dfm->symptom_count = 0;

	#if ((TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN > 0) || (TRC_CFG_DFM_MAX_SYMPTOMS > 0))

	dfm_payload_t* dfm_payload = (dfm_payload_t*)&(RecorderDataPtr->dfm_payload);

	#if (TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN > 0)
	prvStrncpy(dfm_payload->firmware_version, "NO_VERSION", TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN);
	#endif

	#if (TRC_CFG_DFM_MAX_SYMPTOMS > 0)
	for(int i; i<TRC_CFG_DFM_MAX_SYMPTOMS; i++)
	{
		dfm_payload->symptoms[i].symptom_id = 0;
		dfm_payload->symptoms[i].value = 0;
	}
	#endif
...
void dfm_set_firmware_version(const char * firmware_version)
{
	#if TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN > 0
	dfm_payload_t* dfm_payload = (dfm_payload_t*)&(RecorderDataPtr->dfm_payload);

	prvStrncpy(dfm_payload->firmware_version, firmware_version, TRC_CFG_DFM_FIRMWARE_VERSION_MAX_LEN);
	#endif
}

void dfm_alert(uint8_t alert_type)
{
	#if (TRC_CFG_DFM_ENABLED >= 1)
	dfm_header_t* dfm = (dfm_header_t*)&(RecorderDataPtr->dfm_header);

	dfm->alert_type = alert_type;
	#endif
}

void dfm_alert_reset()
{
	#if (TRC_CFG_DFM_ENABLED >= 1)
	dfm_header_t* dfm = (dfm_header_t*)&(RecorderDataPtr->dfm_header);
...
void dfm_add_symptom(uint32_t symptom_id, uint32_t value)
{
	#if (TRC_CFG_DFM_MAX_SYMPTOMS > 0)
	...
	if (dfm->symptom_count < TRC_CFG_DFM_MAX_SYMPTOMS)
	{		
		dfm_payload->symptoms[dfm->symptom_count].symptom_id = symptom_id;
		dfm_payload->symptoms[dfm->symptom_count].value = value;
		dfm->symptom_count++;
	}
